...
export class ResultViewer extends LitElement {
  ...
  private _buildSlideSrcdoc(
    ...
    html = html.replace(
      ...
      (_, path, height) => {
        const h = height || '400';
        return `<iframe src="/${path}?view=form" style="width:100%;height:${h}px;border:none;border-radius:8px;overflow:hidden;background:#111318;color-scheme:dark;" sandbox="allow-scripts allow-same-origin allow-forms allow-popups"></iframe>`;
      }
    );
