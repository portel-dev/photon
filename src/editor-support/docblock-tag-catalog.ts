...
/** Supported class-level @auth spellings, including passwordless methods. */
...
export function buildPhotonDocblockTagCatalog(runtimeVersion: string): PhotonDocblockTagCatalog {
  ...
  const classLevelTags: PhotonDocblockTagDef[] = [
    ...
    {
      label: '@prompt',
      detail: 'Bundle a prompt template file with this photon (class-level)',
      info: 'Class-level form: ships a markdown/text file as an MCP prompt asset. For dynamic prompt templates that take parameters, use the method-level @prompt instead.',
      snippetTmpl: '@prompt ${1:name} ${2:./prompts/prompt.txt}',
      type: 'keyword',
    },
    {
      label: '@resource',
      detail: 'Bundle a static file as an MCP resource (class-level)',
      info: 'Class-level form: ships a file (JSON, markdown, etc.) as an MCP resource asset. For dynamic resources resolved from URI templates, use the method-level @resource instead.',
    ...
    },
    {
      ...
      info: 'Use @auth oauth required or @auth oauth optional for inbound MCP OAuth. Use @auth email passkey for passwordless email sign-in with passkey support and email recovery. Add optional to permit anonymous tools. One-token legacy schemes remain supported.',
      snippetTmpl:
        '@auth ${1|required,optional,oauth required,oauth optional,email,passkey,email passkey,email passkey optional|}',
      type: 'keyword',
    },
    {
      label: '@forkedFrom',
  ...
  const methodLevelTags: PhotonDocblockTagDef[] = [
    ...
    {
      label: '@resource',
      detail: 'Expose this method as a dynamic MCP resource resolver (method-level)',
      info: 'Method-level form: matches resources/read URIs against the URI template and dispatches the method to resolve. Use template parameters in {curly} braces. Example: `@resource person://{slug}`.',
      snippetTmpl: '@resource ${1:scheme://{param\\}}',
      type: 'keyword',
    },
    {
      label: '@prompt',
      detail: 'Expose this method as a dynamic MCP prompt template (method-level)',
      info: 'Method-level form: registers the method as an MCP prompt that the host renders in its slash menu. Method should return a TemplateResponse (`{ messages: [...] }`) or a string.',
      apply: '@prompt',
      type: 'keyword',
    },
    {
      label: '@Static',
      detail: 'DEPRECATED — use @resource (method-level) instead',
      info: 'Legacy uppercase form of method-level @resource. Still accepted for backward compatibility.',
      snippetTmpl: '@Static ${1:scheme://{param\\}}',
      type: 'keyword',
    },
    {
      label: '@Template',
      detail: 'DEPRECATED — use @prompt (method-level) instead',
      info: 'Legacy uppercase form of method-level @prompt. Still accepted for backward compatibility.',
      apply: '@Template',
      type: 'keyword',
    },
    {
