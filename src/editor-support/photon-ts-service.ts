...
import type {
  PhotonTsCompletionKind as CompletionKind,
  PhotonTsWorkerRequest as WorkerRequest,
  PhotonTsWorkerResponse as WorkerResponse,
...
function getCompletions(pos: number, explicit: boolean) {
  ...
  const completions = languageService.getCompletionsAtPosition(currentFilePath, shadowPos, {
    ...
    triggerCharacter: explicit
      ? undefined
      : (currentSource[pos - 1] as ts.CompletionsTriggerCharacter | undefined),
  });
...
function getDefinition(pos: number) {
  ...
  if (!definitionInfo || definitions.length === 0) return null;

  const definition =
    definitions.find((entry) => normalizePath(entry.fileName) === normalizePath(currentFilePath)) ||
    definitions[0];
...
export function createPhotonTsService() {
  return {
    handleRequest(msg: WorkerRequest): WorkerResponse {
      try {
        syncDocument(msg.filePath, msg.source, msg.supportFiles);

        if (msg.type === 'sync') {
          return { id: msg.id, ok: true, result: null };
        }
