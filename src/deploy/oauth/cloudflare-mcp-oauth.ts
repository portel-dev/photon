...
async function photonOAuthConsentPage(tx: any): Promise<Response> {
  ...
  const html = '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Connect ' + clientName + '</title><style>' +
    ':root{color-scheme:light dark;--bg:#f5f7fb;--panel:#fff;--ink:#172033;--muted:#667085;--line:#e5e7eb;--accent:#635bff;--accent-strong:#5147e5;--soft:#eef0ff}*{box-sizing:border-box}body{margin:0;min-height:100vh;background:radial-gradient(circle at 15% 0,#e9e7ff 0,transparent 36%),var(--bg);font:15px/1.5 Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:var(--ink);display:grid;place-items:center;padding:28px 16px}.shell{width:min(100%,640px)}.brand{display:flex;align-items:center;gap:10px;margin:0 0 18px 4px;font-weight:700;letter-spacing:-.01em}.mark{width:32px;height:32px;border-radius:10px;background:linear-gradient(135deg,#635bff,#9b8cff);color:#fff;display:grid;place-items:center;font-weight:800;box-shadow:0 7px 18px #635bff40}.card{background:color-mix(in srgb,var(--panel) 94%,transparent);border:1px solid var(--line);border-radius:24px;box-shadow:0 22px 65px #1720331a;overflow:hidden}.hero{padding:32px 34px 24px;border-bottom:1px solid var(--line)}.eyebrow{color:var(--accent);font-size:12px;font-weight:750;letter-spacing:.08em;text-transform:uppercase}.hero h1{font-size:30px;line-height:1.12;letter-spacing:-.04em;margin:8px 0 10px}.hero p{color:var(--muted);margin:0;max-width:54ch}.identity{display:flex;align-items:center;gap:12px;margin-top:22px;padding:12px 14px;border:1px solid var(--line);border-radius:14px;background:var(--soft)}.avatar{width:34px;height:34px;border-radius:50%;background:#dcd8ff;color:#4438ca;display:grid;place-items:center;font-weight:750}.identity small{display:block;color:var(--muted);font-size:12px}.content{padding:26px 34px 30px}.section-head{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:12px}.section-head h2{font-size:16px;margin:0}.section-head button{border:0;background:none;color:var(--accent);font:inherit;font-size:13px;font-weight:650;cursor:pointer}.permission{display:grid;grid-template-columns:22px 1fr auto;gap:11px;align-items:start;padding:14px 0;border-bottom:1px solid var(--line);cursor:pointer}.permission input{position:absolute;opacity:0}.check{width:20px;height:20px;border:1.5px solid #b6bdca;border-radius:6px;margin-top:1px;position:relative}.permission input:checked+.check{background:var(--accent);border-color:var(--accent)}.permission input:checked+.check:after{content:"";position:absolute;left:6px;top:3px;width:5px;height:9px;border:solid white;border-width:0 2px 2px 0;transform:rotate(45deg)}.permission strong{display:block;font-size:14px}.permission small{display:block;color:var(--muted);font-size:12px;margin-top:2px}.permission code{color:var(--muted);font-size:11px;margin-top:2px}.notice{margin:20px 0 0;padding:12px 14px;border-radius:12px;background:#f8fafc;color:var(--muted);font-size:12px}.actions{display:flex;flex-direction:row-reverse;gap:10px;margin-top:24px}.actions button{border-radius:11px;padding:11px 18px;font:inherit;font-weight:700;cursor:pointer}.allow{border:1px solid var(--accent);background:var(--accent);color:#fff;box-shadow:0 7px 15px #635bff35}.allow:hover{background:var(--accent-strong)}.deny{border:1px solid var(--line);background:transparent;color:var(--ink)}.allow:focus-visible,.deny:focus-visible,.section-head button:focus-visible,.permission:has(input:focus-visible){outline:3px solid #a7a1ff;outline-offset:3px}@media(prefers-color-scheme:dark){:root{--bg:#101118;--panel:#1b1c24;--ink:#f4f5f7;--muted:#a5adbd;--line:#303340;--soft:#292842}.notice{background:#22242d}.avatar{background:#39335f;color:#c7c2ff}}@media(max-width:560px){body{padding:14px}.hero,.content{padding-left:22px;padding-right:22px}.hero h1{font-size:26px}.permission{grid-template-columns:22px 1fr}.permission code{grid-column:2}.actions{flex-direction:column}.actions button{width:100%}}' +
    '</style></head><body><main class="shell"><div class="brand"><span class="mark">P</span><span>Photon secure connection</span></div><section class="card"><header class="hero"><div class="eyebrow">Permission request</div><h1>Connect ' + clientName + '</h1><p>Review what this assistant can access through Photon. You can change these permissions before continuing.</p><div class="identity"><span class="avatar">' + (subject.charAt(0).toUpperCase() || 'A') + '</span><span><strong>' + subject + '</strong><small>Signed-in account · ' + String(tx.role ?? 'user') + '</small></span></div></header><form method="post" action="/consent" class="content"><div class="section-head"><h2>Requested permissions</h2><button type="button" id="toggle">Deselect all</button></div><div id="permissions">' + scopeRows + '</div><div class="notice">You can revoke this connection later from your assistant settings. Only the permissions selected here will be granted.</div><input type="hidden" name="tx" value="' + encodeURIComponent(tx.id) + '"><div class="actions"><button class="allow" name="action" value="approve" type="submit">Allow selected</button><button class="deny" name="action" value="deny" type="submit">Cancel</button></div></form></section></main><script>const boxes=[...document.querySelectorAll("input[name=scope]")],toggle=document.getElementById("toggle");toggle.onclick=()=>{const all=boxes.every(b=>b.checked);boxes.forEach(b=>b.checked=!all);toggle.textContent=all?"Select all":"Deselect all"};boxes.forEach(b=>b.addEventListener("change",()=>{toggle.textContent=boxes.every(x=>x.checked)?"Deselect all":"Select all"}));</script></body></html>';
